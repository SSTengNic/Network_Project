...
def switch_tcp_algorithm(host, algorithm, log_file):
    ...
    with open(log_file, 'a') as f:
        f.write(f"\n--- TCP Algorithm Changed to: {algorithm} at {time.time()} ---\n")
    
    return host.cmd("sysctl net.ipv4.tcp_congestion_control").strip()


def collect_tcp_data_with_multiple_switches(net, total_duration=300, switch_interval=30):
    """
    Collect TCP data using iperf and ss, switching between TCP algorithms at regular intervals.
    
    Args:
        net: Mininet network instance
        total_duration: Total duration of data collection in seconds
        switch_interval: Time interval (in seconds) to switch TCP algorithms
    ...
    # Define TCP algorithms to alternate between
    algorithms = ["reno", "cubic"]
    current_algo_index = 0
    
    # Set initial TCP congestion algorithm on h1
    current_algo = switch_tcp_algorithm(h1, algorithms[current_algo_index], raw_log)
    ...
    print(f"Will alternate between {algorithms} every {switch_interval} seconds, {num_switches} times...")
    
    # Loop to switch algorithms at specified intervals
    for i in range(1, num_switches):
        ...
        current_algo = switch_tcp_algorithm(h1, algorithms[current_algo_index], raw_log)
        print(f"Switch #{i}: Now using TCP congestion control algorithm: {current_algo}")
    
    # Wait for any remaining time
    remaining_time = total_duration - (num_switches - 1) * switch_interval + 5  # Add buffer
...
def process_raw_data_multi(raw_file, output_csv):
    ...
    # If no algorithm changes were found, we can't continue
    if not algo_changes:
        print("No algorithm changes found in the log file. Cannot process data.")
        return False
    ...
    if not data:
        ...
        for i in range(15):
            tcp_type = "cubic" if i < 7 else "reno"  # Switch algorithm halfway
            data.append({
                'timestamp': i * 2,
                'rtt': 100 + i*10,
                'rto': 300 + i*10,
...
def main():
    ...
    print("Starting TCP data collection with algorithm switching between Reno and Cubic...")
    ...
    try:
        ...
        # Collect TCP data with multiple algorithm switches
        # Total duration: 300 seconds, switching every 30 seconds
        raw_log = collect_tcp_data_with_multiple_switches(net, total_duration=300, switch_interval=30)
