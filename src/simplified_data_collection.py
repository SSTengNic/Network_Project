...
"""
Simplified TCP Congestion Control Data Collection using Mininet
This script creates a basic network simulation and collects performance data
for the Cubic TCP congestion control algorithm with a single configuration.
"""
...
class SimpleTopo(Topo):
    ...
    def build(self):
        ...
        # Add this last to avoid interface naming conflicts
        self.addLink(
            s1, 
            s2, 
            bw=10, 
...
def collect_tcp_data(net, duration=30):
    """
    Collect TCP data using iperf and ss.
    ...
    raw_log = f"{data_dir}/raw_cubic_{timestamp}.log"
    
    # Get hosts
    h1, h2 = net.get('h1', 'h2')
    
    # Set TCP congestion algorithm to cubic on h1
    h1.cmd('sysctl -w net.ipv4.tcp_congestion_control=cubic')
    print(f"Set TCP congestion control algorithm to cubic on {h1.name}")
    ...
    # Use a more detailed ss command to ensure we get all metrics
    ...
    print("Started iperf client on h1")
...
def process_raw_data(raw_file, output_csv):
    ...
    for section_idx, section in enumerate(sections[1:]):  # Skip first empty section
        ...
        print(f"\nProcessing section at time {timestamp} seconds:")
        
        # Extract connection lines - each connection spans multiple lines
        # Look for lines with connection details
        lines = section.split('\n')
        for i in range(len(lines)):
            ...
            if i + 1 < len(lines) and 'skmem' in lines[i + 1]:
                ...
                # Extract retrans info - note the format difference!
                ...
                # This is an approximation since the ss output doesn't directly show bytes_sent
                # Assuming bytes_sent is approximately bytes_acked + current unacked data
                ...
                # Add the entry with default values for missing fields
                data.append(metrics)
    ...
    column_order = [
        ...
        'lastrcv', 'delivered', 'rcv_space', 'rcv_ssthresh', 'loss_ratio', 'tcp_type'
    ]
...
def main():
    ...
    print("Starting simplified TCP data collection...")
    ...
    os.makedirs(data_dir, exist_ok=True)
    raw_log = f"{data_dir}/raw_cubic_{timestamp}.log"
    output_csv = f"{data_dir}/cubic_{timestamp}.csv"
    ...
    try:
        ...
        # Collect TCP data
        raw_log = collect_tcp_data(net, duration=30)
